...
* **Validation and Field Extraction**: `TerraformOutputs.from_state_outputs()` extracts scalar values from Terraform's `{"value": ...}` JSON envelope, strips whitespace, and enforces that required attributes are non-empty.
