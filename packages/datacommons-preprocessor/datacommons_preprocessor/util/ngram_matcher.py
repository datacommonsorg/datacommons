...
Example:
  ...
  # [('San Jose California', 'dcid:geoId/0668000'), ('San Jose Costa Rica', 'dcid:wikidataId/Q647808')]
...
class NgramMatcher:
    ...
    def lookup(
        ...
        _DEBUG and logging.debug(f"looking up ngrams {ngrams} for {key}")
            ...
            if ngram_matches:
                ...
                for key_index, ngram_pos in ngram_matches:
                    ...
                    if key_index not in matches:
                        ...
                        key_match["ngram_matches"] = key_match["ngram_matches"] + 1
                        key_match["ngram_pos"] = min(key_match["ngram_pos"], ngram_pos)

        _DEBUG and logging.debug(f"Matches for {key}: {matches}")
        # Collect all key indices that matches with counts.
        match_indices = list()
        min_matches = max(1, len(ngrams) * lookup_config.get("min_match_fraction", 0.8))
        ...
        match_indices.sort(
            key=lambda x: self._get_ngram_match_score(x[1], key_len), reverse=True
        )
        ...
        for match in match_indices:
            ...
            if return_score:
                results.append((result_key, {"value": result_value, "info": match[1]}))
            else:
    ...
    def _add_key_index(self, key: str, key_index: int):
        ...
        for ngram in ngrams:
            ...
            _DEBUG and logging.debug(f'Added ngram "{ngram}" for {key}:{key_index}')
    ...
    def _normalize_string(self, key: str) -> str:
        ...
        if self._config.get("ignore_non_alphanum", True):
            normalized_key = "".join(
