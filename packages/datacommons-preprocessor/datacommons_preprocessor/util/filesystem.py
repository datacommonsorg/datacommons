...
def create_store(
    path: str, create_if_missing: bool = False, treat_as_file: bool = False
) -> "Store":
...
class Store:
    ...
    def __init__(self, path: str, create_if_missing: bool, treat_as_file: bool):
        ...
        if not treat_as_file:
            try:
                ...
                if self.root_path.startswith(_GCS_PATH_PREFIX):
                    self.fs = open_fs(
                        f"{self.root_path}?strict=False", create=create_if_missing
                    )
        ...
        if treat_as_file:
            ...
            self.fs = open_fs(fspath.forcedir(self.root_path), create=create_if_missing)
            self._wrapper: _StoreWrapper = File(
...
def _fix_gcsfs_storage(fs: GCSFS) -> None:
    ...
    """
    names = [
        blob.name for blob in fs.bucket.list_blobs(prefix=fspath.forcedir(fs.root_path))
    ]
    marked_dirs = set()
    all_dirs = set()
