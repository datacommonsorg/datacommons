...
def resolve_entities(
    entities: list[str], entity_type: str = None, property_name: str = "description"
) -> dict[str, str]:
    ...
    if not entity_type or entity_type in _RESOLVE_PLACE_TYPES:
        return resolve_place_entities(
            entities=entities, entity_type=entity_type, property_name=property_name
        )
...
async def _resolve_place_entities_chunk(
    client: AsyncClient, entities_chunk: list[str], entity_type: str, property_name: str
) -> dict[str, str]:
...
def resolve_non_place_entities(
    entities: list[str], entity_type: str = None
) -> dict[str, str]:
    ...
    while True:
        ...
        if ngrams.get_tuples_count() >= _MAX_NODES:
            logging.warning("Nodes fetched truncated to: %s", ngrams.get_tuples_count())
            break
        if next_token:
    ...
    if _DEBUG:
        ...
        logging.info(
            "Writing %s entities to %s for debugging.", entity_type, entities_file
        )
...
def get_entities_of_type(
    entity_type: str, next_token: str = None
...
def get_property_of_entities(entities: list[str], property_name: str) -> dict[str, str]:
    return asyncio.run(get_property_of_entities_async(entities, property_name))


async def get_property_of_entities_async(
    entities: list[str], property_name: str
) -> dict[str, str]:
...
async def _get_property_of_entities_chunk(
    client: AsyncClient, entities_chunk: list[str], property_name: str
) -> dict[str, str]:
    data = {
        "nodes": entities_chunk,
