...
def process_observations(db, output_dir, ns_map: dict, chunk_size: int):
    ...
    args_list = [
        (i, i * chunk_size, chunk_size, db_path, output_dir_path, ns_map, prov_urls)
    ...
    logging.info(
        "Starting observations export with %d processes for %d chunks",
        num_processes,
        num_chunks,
    )
...
def write_shard(g: Graph, index: int, output_dir, ns_map: dict, prefix: str = "output"):
    """
