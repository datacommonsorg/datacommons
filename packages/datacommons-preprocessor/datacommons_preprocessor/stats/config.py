...
class Config:
    ...
    def data_download_urls(self) -> list[str]:
        ...
        if isinstance(cfg, list):
            return cfg
        raise ValueError(f"{_DATA_DOWNLOAD_URL_FIELD} can only be a list, found: {cfg}")
    ...
    def variable(self, variable_name: str) -> StatVar:
        ...
        search_descriptions = var_cfg.get(_SEARCH_DESCRIPTIONS_FIELD, []) + var_cfg.get(
            _NL_SENTENCES_FIELD, []
        )
        return StatVar(
            "",
    ...
    def aggregation(self, variable_name: str) -> AggregationConfig:
        aggregation_cfg = (
            self.data.get(_VARIABLES_FIELD, {})
            .get(variable_name, {})
    ...
    def entity(self, entity_type_name: str) -> EntityType:
        entity_type_cfg = self.data.get(_ENTITIES_FIELD, {}).get(entity_type_name, {})
        return EntityType(
            "",
    ...
    def provenance_name(self, input_file: File) -> str:
        return self._per_file_config(input_file).get(_PROVENANCE_FIELD, input_file.path)
    ...
    def custom_svg_prefix(self) -> str:
        ...
        if cfg:
            ...
            if (
                cfg.startswith("/")
                or not cfg.endswith("/")
                or not re.fullmatch(r"^[A-Za-z0-9_/]+$", cfg)
            ):
                raise ValueError(
    ...
    def default_custom_root_svg_name(self) -> str:
        ...
        return self.data.get(
            _DEFAULT_CUSTOM_ROOT_SVG_NAME_FIELD, sc.DEFAULT_CUSTOM_ROOT_SVG_NAME
        )
    ...
    def _parse_provenances_and_sources(self):
        ...
        for source_name, source_cfg in sources_cfg.items():
            source = Source(id="", name=source_name, url=source_cfg.get(_URL_FIELD, ""))
            provenances = source_cfg.get(_PROVENANCES_FIELD, {})
            for prov_name, prov_url in provenances.items():
                provenance = Provenance(
                    id="", source_id="", name=prov_name, url=prov_url
                )
