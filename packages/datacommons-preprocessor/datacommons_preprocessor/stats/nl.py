...
def generate_nl_sentences(triples: list[Triple], nl_dir: Dir):
    ...
    for triple in triples:
        dcid2candidates.setdefault(triple.subject_id, SentenceCandidates()).maybe_add(
            triple
        )
...
def generate_topic_cache(triples: list[Triple], nl_dir: Dir):
    ...
    for triple in triples:
        dcid2nodes.setdefault(
            triple.subject_id, TopicCacheNode(triple.subject_id)
        ).maybe_add(triple)
    ...
    logging.info("Writing %s topic cache nodes to: %s", len(nodes), topic_cache_file)
