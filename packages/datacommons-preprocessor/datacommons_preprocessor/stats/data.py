...
class Triple:
    ...
    def db_tuple(self):
        return (
            strip_namespace(self.subject_id),
            self.predicate,
            strip_namespace(self.object_id),
            self.object_value,
        )
...
class StatVarGroup:
    ...
    def triples(self) -> list[Triple]:
        ...
        for source_id in self.source_ids:
            triples.append(Triple(self.id, _PREDICATE_INCLUDED_IN, object_id=source_id))
        return triples
...
class StatVar:
    ...
    def triples(self) -> list[Triple]:
        ...
        if self.description:
            triples.append(
                Triple(self.id, _PREDICATE_DESCRIPTION, object_value=self.description)
            )
...
class Source:
    ...
    def triples(self) -> list[Triple]:
        ...
        if self.url:
            triples.append(Triple(self.id, _PREDICATE_URL, object_value=self.url))
        if self.domain:
            triples.append(Triple(self.id, _PREDICATE_DOMAIN, object_value=self.domain))
...
class ObservationProperties:
    ...
    @classmethod
    def new(
        cls: Self, all_properties: dict[str, str] = {}, default_obs_props: Self = None
    ) -> Self:
        if default_obs_props is None:
            default_obs_props = cls()
        unit = all_properties.get(sc.PREDICATE_UNIT, "") or default_obs_props.unit
        scaling_factor = (
            all_properties.get(sc.PREDICATE_SCALING_FACTOR, "")
            or default_obs_props.scaling_factor
        )
        measurement_method = (
            all_properties.get(sc.PREDICATE_MEASUREMENT_METHOD, "")
            or default_obs_props.measurement_method
        )
        observation_period = (
            all_properties.get(sc.PREDICATE_OBSERVATION_PERIOD, "")
...
class Observation:
    ...
    value: str
    provenance: str
    properties: ObservationProperties = field(default_factory=ObservationProperties.new)

    def db_tuple(self):
        return (
            strip_namespace(self.entity),
            strip_namespace(self.variable),
            self.date,
            self.value,
            strip_namespace(self.provenance),
...
class Event:
    ...
    def triples(self) -> list[Triple]:
        ...
        if self.entity:
            triples.append(Triple(self.id, _PREDICATE_LOCATION, object_id=self.entity))
        if self.date:
            triples.append(
...
class VerticalSpec:
    ...
    def from_json(json: dict) -> Self:
        return VerticalSpec(
            population_type=json.get("populationType", sc.DEFAULT_POPULATION_TYPE),
            measured_properties=set(json.get("measuredProperties", [])),
...
def validate_numeric_values(df: pd.DataFrame, file_path: str) -> None:
    ...
    if na_mask.any():
        val_series = (
            df.loc[na_mask, constants.COLUMN_VALUE].astype(str).str.strip().str.lower()
        )
        ...
        if invalid_mask.any():
            invalid_examples = (
                df.loc[na_mask].loc[invalid_mask, constants.COLUMN_VALUE].unique()
            )
            ex = ValueError(
