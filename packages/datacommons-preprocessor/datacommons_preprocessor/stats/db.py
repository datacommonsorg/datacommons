...
_CLOUDSQL_ENV_TO_PARAM = {
    ENV_CLOUDSQL_USE_PRIVATE_IP: lambda v: (
        ("ip_type", IPTypes.PRIVATE) if v.lower() == "true" else None
    ),
}
...
class DataCommonsPlatformDb(Db):
    ...
    def insert_triples(
        ...
        if response.status_code != 200:
            ...
            logging.warning(
                "Failed to write triples to Data Commons Platform: %s", response.text
            )

    def insert_observations(self, observations_df: pd.DataFrame, input_file: File):
        # TODO: Implement observation insertion into Data Commons Platform.
        logging.info(
            "TODO: Writing %s observations to [%s]", len(observations_df), self.url
        )
        pass
...
class DbEngine:
    ...
    def get_row_counts(self) -> dict:
        ...
        return {
            ...
            "key_value_store": self.fetch_all("SELECT COUNT(*) FROM key_value_store")[
                0
            ][0],
        }
...
class SqliteDbEngine(DbEngine):
    def __init__(self, db_params: dict) -> None:
        ...
        if self.db_file.syspath() is None:
            logging.info(
                "Copying DB to local storage from %s", self.db_file.full_path()
            )
            # Copy to disk
...
class CloudSqlDbEngine(DbEngine):
    def __init__(self, db_params: dict[str, str]) -> None:
        ...
        logging.info(
            "Connecting to Cloud MySQL: %s (%s)",
            db_params[CLOUD_MY_SQL_INSTANCE],
            db_params[CLOUD_MY_SQL_DB],
        )
    ...
    def _maybe_create_database(connector: Connector, db_params: dict[str, str]) -> None:
        ...
        kwargs = {
            param: db_params[param] for param in _CLOUD_MY_SQL_INSTANCE_CONNECT_PARAMS
        } | _get_optional_cloudsql_params()
...
def create_sqlite_config(sqlite_db_file: File) -> dict:
    return {
        ...
        FIELD_DB_PARAMS: {SQLITE_DB_FILE: sqlite_db_file},
    }
...
def get_datacommons_platform_config_from_env() -> dict | None:
    ...
    assert dcp_url, (
        f"Environment variable {ENV_DATA_COMMONS_PLATFORM_URL} not specified."
    )
    return {
...
def get_blue_green_config_from_env() -> dict:
    ...
    config = {
        ...
        "local_sqlite_path": os.getenv(
            "LOCAL_BUILD_SQLITE_PATH", "/tmp/datacommons_build.db"
        ),
    }
