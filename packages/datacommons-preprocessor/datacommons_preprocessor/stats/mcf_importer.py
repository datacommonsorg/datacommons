...
from datacommons_preprocessor.kg_util.mcf_parser import mcf_to_triples
import pandas as pd
        ...
        try:
            ...
            if self.is_main_dc:
                ...
                for parser_triple in triples_list:
                    ...
                    if (
                        subject_id in metadata_subject_ids
                        or resolved_subject in metadata_subject_ids
                    ):
                    ...
                    if (
                        chunk
                        and triple.subject_id != chunk[-1].subject_id
                        and len(chunk) >= 10000
                    ):
                ...
                if all_metadata_triples:
                    ...
                    _register_metadata_nodes(
                        all_metadata_triples, self.nodes, provenance_id=prov_id
                    )
...
def _register_metadata_nodes(
    triples: list[Triple], nodes: Nodes, provenance_id: str = ""
) -> None:
