...
class EventsImporter(Importer):
    ...
    def __init__(
        ...
        assert self.event_type, (
            f"Event type must be specified: {self.input_file.full_path()}"
        )
    ...
    def _write_observations(self) -> None:
        ...
        if not sv_names:
            logging.warning(
                "No computed variables specified: %s", self.input_file.full_path()
            )
            return
    ...
    def _compute_sv_observations(
        ...
        )[constants.COLUMN_DCID].transform("count")
    ...
    def _write_event_triples(self) -> None:
        ...
        id_column_name = (
            self.nodes.property(self.id_column).dcid if self.id_column else ""
        )
    ...
    def _write_debug_csvs(self) -> None:
        if self.debug_resolve_df is not None:
            logging.info(
                "Writing resolutions (for debugging) to: %s", self.debug_resolve_file
            )
