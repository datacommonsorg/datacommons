...
class MetadataValidator:
    ...
    def validate(self) -> None:
        ...
        self._validate_provenance_definitions(
            referenced_provenances, defined_provenances
        )
    ...
    def _collect_defined_nodes(self) -> tuple[set[str], dict[str, str]]:
        ...
        if hasattr(self.db, "nodes") and self.db.nodes:
            for prov_id, prov in self.db.nodes.provenances.items():
                ...
                if prov.source_id:
                    provenance_to_source[clean_prov_id] = self._clean_dcid(
                        prov.source_id
                    )
    ...
    def _validate_provenance_definitions(
        self, referenced: set[str], defined: set[str]
    ) -> None:
    ...
    def _validate_source_links(
        self, defined_provs: set[str], links: dict[str, str]
    ) -> None:
