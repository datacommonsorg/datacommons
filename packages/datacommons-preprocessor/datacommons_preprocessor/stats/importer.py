...
class EntityResolutionError(ValueError):
    ...
    def __init__(
        ...
        self.unresolved_entities = (
            unresolved_entities if unresolved_entities is not None else []
        )
...
class Importer:
    ...
    def check_and_report_unresolved_entities(
        self, unresolved_entities: set[str]
    ) -> None:
    ...
    def resolve_specified_columns(
        ...
        for col_name in cols_to_resolve:
            ...
            column = column.map(lambda x: dcids.get(x, pre_resolved_entities.get(x, x)))
            df[target_col] = column
