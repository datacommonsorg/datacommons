...
def generate(
    ...
    *,
    ...
    return _generate_internal(
        triples, vertical_specs, dcid2name, svg_prefix, root_svg_id, blocklist
    ).to_result()
...
def _generate_internal(
    ...
    sv_hierarchy_props_blocklist: set[str] | None = None,
) -> "StatVarHierarchy":
    """Given a list of input triples (including stat vars),
    ...
    vertical_svgs = _attach_verticals(
        pop_type_svgs,
        vertical_specs,
        dcid2name,
        custom_svg_prefix,
...
class PropVal:
    ...
    def gen_pv_name(self, dcid2name: dict[str, str]) -> str:
        if self.val:
            return (
                f"{_gen_name(self.prop, dcid2name)} = {_gen_name(self.val, dcid2name)}"
            )
...
class SVPropVals:
    ...
    def gen_svg_id(self, custom_svg_prefix: str = sc.CUSTOM_SVG_PREFIX):
        ...
        if len(svg_id) > MAX_SVG_ID_LENGTH:
            ...
            hash_suffix = hashlib.sha256(svg_id.encode()).hexdigest()[
                :SVG_ID_HASH_LENGTH
            ]
    ...
    def gen_specialized_name(self, parent_pvs: Self, dcid2name: dict[str, str]) -> str:
...
class SVG:
    ...
    def triples(self) -> list[Triple]:
        ...
        for parent_svg_id in self.parent_svg_ids.keys():
            triples.append(
                Triple(
                    self.svg_id, sc.PREDICATE_SPECIALIZATION_OF, object_id=parent_svg_id
                )
            )

        # SV members ("memberOf")
        for sv_id in self.sv_ids:
            triples.append(Triple(sv_id, sc.PREDICATE_MEMBER_OF, object_id=self.svg_id))

        return triples

    def gen_specialized_name(self, parent_svg: Self, dcid2name: dict[str, str]) -> str:
        if self.sample_sv and parent_svg.sample_sv:
            return self.sample_sv.gen_specialized_name(parent_svg.sample_sv, dcid2name)
        return ""
...
class StatVarHierarchy:
    ...
    def to_result(self) -> StatVarHierarchyResult:
        return StatVarHierarchyResult(
            self.svg_triples, self._get_svg_specialized_names()
        )
...
def _attach_verticals(
    poptype2svg: dict[str, SVG],
    vertical_specs: list[VerticalSpec],
    dcid2name: dict[str, str],
    custom_svg_prefix: str,
    default_custom_root_svg_id: str,
) -> dict[str, SVG]:
    ...
    for vertical_spec in vertical_specs:
        ...
        if not (
            vertical_spec.measured_properties & pop_type_svg.measured_properties.keys()
        ):
            continue
        # Put pop type svg under all verticals in the spec.
        for vertical in vertical_spec.verticals:
            vertical_svg = _get_or_create_vertical_svg(
                vertical,
                vertical_svgs,
                dcid2name,
                custom_svg_prefix,
...
def _get_or_create_vertical_svg(
    vertical: str,
    vertical_svgs: dict[str, SVG],
    dcid2name: dict[str, str],
    custom_svg_prefix: str,
...
def _get_pop_type_svgs(svgs: dict[str, SVG]) -> dict[str, SVG]:
    ...
    for svg in svgs.values():
        if not svg.parent_svg_ids and (svg.sample_sv and svg.sample_sv.population_type):
            poptype2svg[svg.sample_sv.population_type] = svg
    return poptype2svg


def _get_or_create_svg(
    svgs: dict[str, SVG],
    sv: SVPropVals,
    dcid2name: dict[str, str],
    custom_svg_prefix: str,
...
def _create_all_svgs(
    svs: list[SVPropVals], dcid2name: dict[str, str], custom_svg_prefix: str
) -> dict[str, SVG]:
...
def _add_measured_properties_to_parent_svgs(
    mprops: dict[str, bool], parent_svg_ids: dict[str, bool], svgs: dict[str, SVG]
):
    ...
    for parent_svg_id in parent_svg_ids:
        ...
        _add_measured_properties_to_parent_svgs(mprops, parent_svg.parent_svg_ids, svgs)


def _create_parent_svg(
    parent_sv: SVPropVals,
    svg: SVG,
    svgs: dict[str, SVG],
    ...
    parent_svg.child_svg_id_2_specialized_name[svg.svg_id] = svg.gen_specialized_name(
        parent_svg, dcid2name
    )

    # Add child mprops to all parents recursively.
    _add_measured_properties_to_parent_svgs(
        svg.measured_properties, svg.parent_svg_ids, svgs
    )
...
def _create_parent_svgs(
    svg_id: str, svgs: dict[str, SVG], dcid2name: dict[str, str], custom_svg_prefix: str
):
    svg = svgs[svg_id]
    sv = svg.sample_sv
    ...
    if svg.has_prop_without_val:
        ...
        _create_parent_svg(
            parent_sv=sv.with_pvs(parent_pvs),
            svg=svg,
            svgs=svgs,
...
def _extract_svs(
    triples: list[Triple], sv_hierarchy_props_blocklist: set[str] | None = None
) -> list[SVPropVals]:
