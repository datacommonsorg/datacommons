...
STANDARD_PROPERTY_MAPPING = {
    ...
    "dcid:measurementMethod": constants.COLUMN_MEASUREMENT_METHOD,  # "measurement_method"
    "dcid:observationPeriod": constants.COLUMN_OBSERVATION_PERIOD,  # "observation_period"
...
class VariablePerRowImporter(Importer):
    ...
    def __init__(
        self, input_file: File, db: Db, reporter: FileImportReporter, nodes: Nodes
    ) -> None:
    ...
    def _validate_mappings(
        self, column_mappings: dict[str, str], custom_dimensions: list[str]
    ...
    def _serialize_custom_dimensions(
        self, df: pd.DataFrame, static_props: dict[str, str]
    ) -> pd.DataFrame:
    ...
    def _add_entity_nodes(self) -> None:
        ...
        if dcid2type:
            logging.info(
                "Importing %s of %s entities.", len(dcid2type), len(new_entity_dcids)
            )
