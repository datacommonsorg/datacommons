...
class EntitiesImporter(Importer):
    ...
    def __init__(
        ...
        assert self.row_entity_type, (
            f"Row entity type must be specified: {self.input_file.full_path()}"
        )
    ...
    def _read_csv(self) -> None:
        ...
        self.df = pd.read_csv(
            self.input_file.read_string_io(), skipinitialspace=True, thousands=","
        )
    ...
    def _write_row_entity_triples(self) -> None:
        ...
        for index, row in self.df.iterrows():
            ...
            dcid = (
                row[id_column_name]
                if id_column_name
                else f"{self.row_entity_type}_{index}"
            )
