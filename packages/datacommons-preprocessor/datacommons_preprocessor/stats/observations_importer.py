...
class ObservationsImporter(Importer):
        ...
        with self.input_file.open_stream() as stream:
            ...
            for chunk_df in reader:
                ...
                if first_chunk:
                    ...
                    renamed.update(
                        {col: id for col, id in zip(sv_column_names, sv_ids)}
                    )
                    first_chunk = False
    ...
    def _write_observations(self) -> None:
        ...
        observations_df = prepare_observations_df(
            observations_df, provenance, obs_props
        )
    ...
    def _add_entity_nodes(self) -> None:
        ...
        if dcid2type:
            logging.info(
                "Importing %s of %s proxy nodes.", len(dcid2type), len(new_entity_dcids)
            )
            self.nodes.entities_with_types(dcid2type, provenance_id=prov_id)
        elif self.entity_type:
            logging.info(
                "Importing %s entities with type %s.",
                len(new_entity_dcids),
                self.entity_type,
            )
            self.nodes.entities_with_type(
                new_entity_dcids, self.entity_type, provenance_id=prov_id
            )
    ...
    def _do_resolve(self, entities: list[str]) -> dict[str, str]:
        ...
        return dc.resolve_entities(
            entities=entities, entity_type=self.entity_type, property_name=property_name
        )
    ...
    def _create_debug_resolve_dataframe(
        ...
        if self.debug_resolve_df is None:
            ...
            self.debug_resolve_df = pd.concat(
                [self.debug_resolve_df, new_df], ignore_index=True
            ).drop_duplicates()

    def _write_debug_csvs(self) -> None:
        if self.debug_resolve_df is not None:
            logging.info(
                "Writing resolutions (for debugging) to: %s",
