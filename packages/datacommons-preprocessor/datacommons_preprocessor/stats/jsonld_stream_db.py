...
def _write_observation_shard(
    ...
    for row in chunk:
        ...
        key = (
            f"{entity}_{variable}_{date}_{provenance}_{unit}_{mmethod}_{period}_{props}"
        )
        ...
        if props:
            try:
                ...
                if isinstance(props_dict, dict):
                    prop_keys = [
                        f"dcid:{k}"
                        if not k.startswith(("dcid:", "http://", "https://"))
                        else k
            ...
            try:
                ...
                if isinstance(props_dict, dict):
                    for k, v in props_dict.items():
                        prop_key = (
                            f"dcid:{k}"
                            if not k.startswith("dcid:") and not k.startswith("http")
                            else k
                        )
...
def _write_node_shard_fast(args):
    ...
    for row in chunk:
        ...
        if sub_id not in subjects:
            subjects[sub_id] = {
                "@id": sub_id
                if is_uri_or_namespace(sub_id)
                else f"dcid:{sub_id.lstrip('/')}"
            }
...
class JsonLdStreamDb(Db):
    ...
    def __init__(
        self, output_dir, import_names, nodes, jsonld_dir_name: Optional[str] = None
    ) -> None:
        ...
        self.import_name = (
            import_name or nodes.config.data.get("importName") or "default_import_name"
        )
            ...
            with self.lock:
                for obs_hash, ent, var, dt, prov in chunk_items:
                    ...
                    if hash_int in self.obs_hash_set:
                        ...
                        sample_info = f"@id='dcid:obs_{obs_hash}' [entity='{ent}', variable='{var}', date='{dt}', provenance='{prov}']"
    ...
    def _get_prov_urls(self) -> dict[str, str]:
        if (
            hasattr(self, "nodes")
            and self.nodes
            and hasattr(self.nodes, "get_provenance_urls")
        ):
            return self.nodes.get_provenance_urls()
        return {}

    def _write_observations_df_to_disk(
        self, df: pd.DataFrame, import_name: str, file_name: str = ""
    ):
    ...
    def insert_observations(self, observations_df: pd.DataFrame, input_file: File):
        ...
        self._write_observations_df_to_disk(
            observations_df, import_name, file_name=file_name
        )
    ...
    def insert_triples(
        self, triples: list[Triple], input_file: File = None, provenance_dir: str = None
    ):
        if not triples:
            return
    ...
    def commit_and_close(self):
        ...
        if global_triples:
            target_import = (
                self.import_name
                if self.import_name in self._processed_imports
        ...
        has_local_files = (
            any(os.scandir(self.temp_local_dir))
            if os.path.exists(self.temp_local_dir)
            else False
        )
        if has_local_files:
        ...
        try:
            ...
            logging.info(
                "Cleaned up local temporary directory: %s", self.temp_local_dir
            )
        except Exception as e:
            logging.warning(
                "Failed to clean up local temporary directory %s: %s",
                self.temp_local_dir,
                e,
            )
    ...
    def _upload_shards_gcs(
        self, temp_local_dir: str, files: list[str], target_path: str
    ):
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=_UPLOAD_CONCURRENCY
        ) as executor:
