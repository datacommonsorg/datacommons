...
class ImportReporter:
    ...
    def save(self) -> None:
        self.last_update = datetime.now()
        should_report = (
            not self.last_reported
            or time.time() - self.last_reported >= _REPORT_SAVE_INTERVAL_SECONDS
            or _is_done_status(self.status)
        )
        if should_report:
...
class FileImportReporter:
    ...
    def __init__(self, import_file_full_path: str, parent: ImportReporter) -> None:
        self.status = Status.NOT_STARTED
        self.start_time = None
        self.last_update = datetime.now()
