...
_DEFAULT_CUSTOM_GROUP = StatVarGroup(
    sc.DEFAULT_CUSTOM_ROOT_SVG_ID, sc.DEFAULT_CUSTOM_ROOT_SVG_NAME, sc.ROOT_SVG_ID
)

_DEFAULT_SOURCE = Source(f"{_CUSTOM_SOURCE_ID_PREFIX}default", "Custom Data Commons")
_DEFAULT_PROVENANCE = Provenance(
    id=f"{_CUSTOM_PROVENANCE_ID_PREFIX}default",
...
class Nodes:
    ...
    def _source_id(self, source_cfg: Source | None) -> str:
        ...
        if not source:
            source = Source(
                id=f"{_CUSTOM_SOURCE_ID_PREFIX}{len(self.sources)}",
    ...
    def variable(self, sv_column_name: str, input_file: File) -> StatVar:
        ...
        return self._add_provenance(
            self.variables[sv_column_name], self.provenance(input_file)
        )

    @thread_safe
    def property(self, property_column_name: str, provenance_id: str = "") -> Property:
    ...
    def group(self, group_path: str) -> StatVarGroup | None:
        ...
        for index in range(len(tokens)):
            ...
            if path not in self.groups:
                ...
                parent_id = (
                    self.groups[parent_path].id
                    if parent_path in self.groups
                    else sc.ROOT_SVG_ID
                )
                svg = StatVarGroup(
    ...
    @thread_safe
    def entity_with_type(
        self, entity_dcid: str, entity_type: str, provenance_id: str = ""
    ):
    ...
    @thread_safe
    def entities_with_type(
        self, entity_dcids: list[str], entity_type: str, provenance_id: str = ""
    ):
        for entity_dcid in entity_dcids:
            self.entity_with_type(entity_dcid, entity_type, provenance_id=provenance_id)

    @thread_safe
    def entities_with_types(self, dcid2type: dict[str, str], provenance_id: str = ""):
        """
    ...
    def triples(self, triples_file: File | None = None) -> list[Triple]:
        ...
        for source in self.sources.values():
            if (
                self.has_custom_mcf_nodes
                and source.id == _DEFAULT_SOURCE.id
                and _DEFAULT_SOURCE.id not in self._used_source_ids
            ):
                continue
        ...
        for provenance in self.provenances.values():
            if (
                self.has_custom_mcf_nodes
                and provenance.id == _DEFAULT_PROVENANCE.id
                and _DEFAULT_PROVENANCE.id not in self._used_provenance_ids
            ):
                continue
