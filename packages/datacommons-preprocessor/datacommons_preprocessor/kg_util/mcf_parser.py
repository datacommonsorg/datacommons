...
def _is_common_ref_property(prop):
    return _is_schema_ref_property(prop) or prop in (
        "location",
        "observedNode",
        "containedInPlace",
        "containedIn",
...
def _is_global_ref(value):
    return (
        value.startswith(_dcid_prefix)
        or value.startswith(_dcs_prefix)
...
def _parse_value(prop, value, pc):
    ...
    if value.startswith('"'):
        assert len(value) > 2 and value.endswith('"'), _as_err("malformed string", pc)
...
def _update_ns_map(values_str, pc):
    ...
    for values in reader([values_str]):
        for value in values:
            ...
            k = parts[0] + _delim
            assert k not in pc.ns_map, _as_err(
                "duplicate values for namespace prefix " + k, pc
            )
...
def mcf_to_triples(mcf_file):
    ...
    for line in mcf_file:
        ...
        assert len(parts) == 2, _as_err("Malformed line without a colon delimiter", pc)
        ...
        if prop == _context:
            ...
            assert not val_str.startswith('"'), _as_err(
                'Malformed Node name starting with "', pc
            )
            ...
            assert prop == _namespace, _as_err(
                'Context block only supports "namespace" property', pc
            )
            ...
            try:
                ...
                raise ValueError(
                    f"Error parsing property {prop} in node {pc.node}"
                ) from e
