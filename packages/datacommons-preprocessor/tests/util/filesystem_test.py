...
class TestFilesystem(unittest.TestCase):
    ...
    def test_create_store_file_new(self):
        with create_store(
            "mem://foo.txt", create_if_missing=True, treat_as_file=True
        ) as store:
        ...
        with create_store(
            "mem://path/to/foo.txt", create_if_missing=True, treat_as_file=True
        ) as store:
            self.assertFalse(store.isdir())
    ...
    def test_create_store_file_existing(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # treat_as_file=True
            with create_store(
                file_path, create_if_missing=False, treat_as_file=True
            ) as store:
                self.assertFalse(store.isdir())
    ...
    def test_file(self):
        with create_store(
            "mem://dir/foo.txt", create_if_missing=True, treat_as_file=True
        ) as store:
            file = store.as_file()
            file.write("hello")
