...
class TestJsonLdStreamDb(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_config.import_name = lambda f: (
            f.path.split("/")[0] if (f and "/" in f.path) else "test_import"
        )
    ...
    def test_directory_creation(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(
                db.jsonld_dir.full_path().startswith(
    ...
    def test_node_fast_vs_rdflib_parity(self):
        ...
        from datacommons_preprocessor.stats.jsonld_stream_db import (
            _write_node_shard_fast,
        )
        from datacommons_preprocessor.stats.jsonld_stream_db import (
            _write_node_shard_rdflib,
        )

        complex_triples = [
            Triple(
                subject_id="sub1", predicate="typeOf", object_id="StatisticalVariable"
            ),
            Triple(subject_id="sub1", predicate="typeOf", object_id="Thing"),
            Triple(subject_id="sub1", predicate="name", object_value="Test Node"),
            # Multi-valued properties
            Triple(
                subject_id="sub1", predicate="alternateName", object_value="Alias A"
            ),
            Triple(
                subject_id="sub1", predicate="alternateName", object_value="Alias B"
            ),
        ...
        with (
            tempfile.TemporaryDirectory() as temp_dir_fast,
    ...
    def test_observation_parsing_edge_cases(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                obs1["http://schema.org/url"], {"@id": "https://test-prop.org"}
            )
    ...
    def test_custom_namespace_rewriting(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                triples = [
                    Triple(
                        "undata:provenance/WHO", "typeOf", object_id="dcid:Provenance"
                    ),
                    Triple("undata:provenance/WHO", "dcid:name", object_value="WHO"),
                    Triple(
                        "undata:provenance/WHO",
                        "schema:url",
