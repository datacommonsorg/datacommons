...
_TEST_DATA_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "test_data", "observations_importer"
)
...
def _test_import(test: unittest.TestCase, test_name: str):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        input_file = input_store.as_dir().open_dir(test_name).open_file(input_file_name)
...
class TestObservationsImporter(unittest.TestCase):
    ...
    def test_custom_namespace_pre_resolution(self):
        config = Config(
            {
                "inputFiles": [
                    {"pattern": "data.csv", "provenance": "undata:provenance/WHO"}
                ]
            }
        )
        nodes = Nodes(config)
        ...
        with (
            ...
            mock.patch.object(dc_client, "get_property_of_entities", return_value={}),
        ):
    ...
    def test_add_entity_nodes_enabled(self):
        ...
        with mock.patch.object(
            ...
            return_value={"country/USA": "Country", "country/FRA": "Country"},
        ) as mock_get_props:
            importer._add_entity_nodes()
