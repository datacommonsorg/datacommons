...
_TEST_DATA_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "test_data", "nodes"
)
_EXPECTED_DIR = os.path.join(_TEST_DATA_DIR, "expected")

CONFIG_DATA = {
    "inputFiles": [
        {"pattern": "a.csv", "entityType": "Country", "provenance": "Provenance1"},
        {"pattern": "b.csv", "entityType": "", "ignoreColumns": ["ignore1", "ignore2"]},
        {
            "pattern": "events.csv",
            "entityType": "Country",
        ...
        {"pattern": "x.csv", "entityType": "", "provenance": "Provenance1"},
    ],
    "variables": {
    ...
    "events": {
        "CrimeEvent": {"name": "Crime Event", "description": "Crime Event description"}
    },
    "entities": {
        "FooEntity": {"name": "Foo Entity", "description": "Foo Entity description"}
    },
    "sources": {
        "Source1": {
            "url": "http://source1.com",
            "provenances": {"Provenance1": "http://source1.com/provenance1"},
        }
    },
}
...
TEST_EVENT_TYPE_INPUT_FILE_NAMES = [
    ("CrimeEvent", "events.csv"),
    ("Event no config", "x.csv"),
]

TEST_ENTITY_TYPE_INPUT_FILE_NAMES = [
    ("FooEntity", "entities.csv"),
...
class TestNodes(unittest.TestCase):
    ...
    def test_triples(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            nodes.triples(
                create_store(
                    output_path, create_if_missing=True, treat_as_file=True
                ).as_file()
            )
    ...
    def test_multiple_parent_groups(self):
        ...
        cfg = Config(
            {
                "inputFiles": [{"pattern": "x.csv", "provenance": "dcid:Provenance1"}],
                "sources": {
                    "Source1": {
                        "url": "http://source1.com",
                        "provenances": {"Provenance1": "http://source1.com/p1"},
                    }
                },
            }
        )
    ...
    def test_default_custom_root_group_name_override(self):
        ...
        default_groups = [
            g for g in nodes.groups.values() if g.id == sc.DEFAULT_CUSTOM_ROOT_SVG_ID
        ]
