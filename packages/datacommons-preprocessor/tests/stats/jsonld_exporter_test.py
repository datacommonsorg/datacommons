    ...
    def test_export(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(shard_paths[0], "r") as f:
                ...
                self.assertEqual(
                    nodes["dcid:sub1"]["@type"], "dcid:StatisticalVariable"
                )
            ...
            with open(shard_paths[3], "r") as f:
                ...
                self.assertEqual(
                    nodes[obs_node_id]["dcid:provenanceUrl"], "http://example.com/p1"
                )
    ...
    def test_double_prefixing(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            db.insert_triples(
                [
                    Triple(
                        subject_id="https://example.com/sub1",
                        predicate="typeOf",
                        object_id="StatisticalVariable",
                    ),
                    Triple(
                        subject_id="dcid:sub2", predicate="name", object_value="Name2"
                    ),
                ]
            )
            db.commit()
