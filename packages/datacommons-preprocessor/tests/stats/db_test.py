...
_TEST_DATA_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "test_data", "db"
)
...
_OBSERVATIONS = [
    ...
    Observation(
        ...
        "789",
        "p1",
        properties=ObservationProperties.new({"unit": "USD", "prop1": "val1"}),
    ),
]
...
_OLD_IMPORT_TUPLE = ("2022-02-02 00:00:00", "SUCCESS", '{"numVars": 1, "numObs": 3}')

# The import previously recorded in sqlite_current_schema_populated.sql
_DIFFERENT_IMPORT_TUPLE = (
    "2021-03-03 00:00:00",
    "SUCCESS",
    '{"numVars": 2, "numObs": 2}',
)
...
class TestDb(unittest.TestCase):
    ...
    def test_sql_db_reimport_without_schema_update(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            db_file_path = os.path.join(temp_dir, db_file_name)
            db_file = temp_store.as_dir().open_file(db_file_name)
            self._seed_db_from_input(
                db_file_path, "sqlite_current_schema_populated.sql"
            )
    ...
    def test_main_dc_db(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected_observations_file = os.path.join(_EXPECTED_DIR, "observations.csv")
            ...
            db.insert_observations(
                _observations_to_df(_OBSERVATIONS), observations_file
            )
            db.insert_import_info(status=ImportStatus.SUCCESS)
            db.commit_and_close()
    ...
    @mock.patch.dict(os.environ, {"USE_CLOUDSQL": "true", "CLOUDSQL_INSTANCE": ""})
    def test_get_cloud_sql_config_from_env_invalid(self):
        with self.assertRaisesRegex(
            AssertionError, "Environment variable CLOUDSQL_INSTANCE not specified."
        ):
            get_cloud_sql_config_from_env()
    ...
    def test_get_datacommons_platform_config_from_env(self):
        self.assertEqual(
            get_datacommons_platform_config_from_env(),
            {
                ...
                "params": {"data_commons_platform_url": "https://test_url"},
            },
        )
