...
_TEST_DATA_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "test_data", "nl"
)
...
def _test_generate_nl_sentences(
    test: unittest.TestCase, test_name: str, generate_topic_cache: bool = False
):
    test.maxDiff = None
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        expected_sentences_csv_path = os.path.join(
            _EXPECTED_DIR, test_name, "sentences.csv"
        )

        output_catalog_yaml_path = os.path.join(
            temp_dir, "embeddings", "custom_catalog.yaml"
        )
        expected_catalog_yaml_path = os.path.join(
            _EXPECTED_DIR, test_name, "custom_catalog.yaml"
        )

        output_topic_cache_json_path = os.path.join(
            temp_dir, "custom_dc_topic_cache.json"
        )
        expected_topic_cache_json_path = os.path.join(
            _EXPECTED_DIR, test_name, "custom_dc_topic_cache.json"
        )

        # Sentences are not generated for StatVarPeerGroup triples.
        # So remove them first.
        nl.generate_nl_sentences(
            _without_svpg_triples(input_triples), nl_dir=temp_store.as_dir()
        )
        ...
        if generate_topic_cache:
            ...
            # So remove them first.
            nl.generate_topic_cache(
                _without_sv_triples(input_triples), nl_dir=temp_store.as_dir()
            )
        ...
        if generate_topic_cache:
            compare_files(
                test, output_topic_cache_json_path, expected_topic_cache_json_path
            )
...
def _without_svpg_triples(triples: list[Triple]) -> list[Triple]:
    ...
    for triple in triples:
        if (
            triple.predicate == sc.PREDICATE_TYPE_OF
            and triple.object_id == sc.TYPE_STAT_VAR_PEER_GROUP
        ):
            svpg_dcids.add(triple.subject_id)

    if not svpg_dcids:
        return triples
    return list(filter(lambda triple: triple.subject_id not in svpg_dcids, triples))
...
def _without_sv_triples(triples: list[Triple]) -> list[Triple]:
    ...
    for triple in triples:
        if (
            triple.predicate == sc.PREDICATE_TYPE_OF
            and triple.object_id == sc.TYPE_STATISTICAL_VARIABLE
        ):
...
class TestData(unittest.TestCase):
    ...
    def test_generate_nl_sentences_for_topic_triples(self):
        _test_generate_nl_sentences(self, "topic_triples", generate_topic_cache=True)

    def test_generate_nl_sentences_for_sv_and_topic_triples(self):
        _test_generate_nl_sentences(
            self, "sv_and_topic_triples", generate_topic_cache=True
        )
