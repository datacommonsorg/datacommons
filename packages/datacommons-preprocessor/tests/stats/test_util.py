...
def compare_files(
    test: unittest.TestCase, actual_path: str, expected_path: str, message: str = None
):
    """
...
def compare_csv_files(
    ...
    actual_empty = (
        not os.path.exists(actual_path) or not open(actual_path).read().strip()
    )
    expected_empty = (
        not os.path.exists(expected_path) or not open(expected_path).read().strip()
    )
    ...
    expected_sorted = expected_df.sort_values(by=list(expected_df.columns)).reset_index(
        drop=True
    )
...
def write_observations(db_path: str, output_path: str):
    ...
    with sqlite3.connect(db_path) as db:
        ...
        pd.DataFrame(rows, columns=OBSERVATION_FIELD_NAMES).to_csv(
...
def write_key_values(db_path: str, output_path: str):
    ...
    with sqlite3.connect(db_path) as db:
        ...
        pd.DataFrame(rows, columns=["lookup_key", "value"]).to_csv(
            output_path, index=False
        )
