...
from datacommons_preprocessor.kg_util import mcf_parser
from parameterized import parameterized
from datacommons_preprocessor.stats.data import Triple
from datacommons_preprocessor.stats.stat_var_hierarchy_generator import *
from datacommons_preprocessor.stats.stat_var_hierarchy_generator import _extract_svs
from datacommons_preprocessor.stats.stat_var_hierarchy_generator import (
    _generate_internal,
)
from datacommons_preprocessor.stats.stat_var_hierarchy_generator import PropVal
from datacommons_preprocessor.stats.stat_var_hierarchy_generator import SVPropVals
...
def _mcf_to_triples(mcf_path: str) -> list[Triple]:
    with open(mcf_path, "r") as mcf_file:
        return list(
            map(
                lambda mcf_triple: _to_triple(mcf_triple),
                mcf_parser.mcf_to_triples(mcf_file),
            )
        )


def _test_generate_internal(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if has_schema_names:
            input_schema_names_path = os.path.join(
                _INPUT_DIR, f"{test_name}.schema_names.json"
            )
        ...
        expected_svgs_json_path = os.path.join(_EXPECTED_DIR, f"{test_name}_svgs.json")

        output_triples_csv_path = os.path.join(temp_dir, f"{test_name}_triples.csv")
        expected_triples_csv_path = os.path.join(
            _EXPECTED_DIR, f"{test_name}_triples.csv"
        )
        ...
        compare_files(
            test,
            output_svgs_json_path,
            expected_svgs_json_path,
            f"Comparing SVGS JSON: {test_name}",
        )
        compare_files(
            test,
            output_triples_csv_path,
            expected_triples_csv_path,
...
class TestStatVarHierarchyGenerator(unittest.TestCase):
    ...
    def test_extract_svs(self):
        ...
        expected_svs = [
            SVPropVals(
                ...
                pvs=[PropVal("gender", "Female"), PropVal("race", "Asian")],
            ...
            SVPropVals(
                sv_id="sv3", population_type="Thing", pvs=[], measured_property=""
            ),
        ]
        ...
        [
            (
                SVPropVals(
                    sv_id="",
                    population_type="",
                    pvs=[PropVal("gender", ""), PropVal("race", "Asian")],
                    measured_property="",
                ),
                SVPropVals(
                    sv_id="",
            ...
            (
                ...
                SVPropVals(
                    sv_id="",
                    population_type="",
                    pvs=[PropVal("gender", "Female"), PropVal("race", "")],
                    measured_property="",
                ),
                {},
                "Race",
            ...
            (
                ...
                SVPropVals(
                    sv_id="",
                    population_type="",
                    pvs=[PropVal("gender", "Female"), PropVal("povertyStatus", "")],
                    measured_property="",
                ),
            ...
            (
                ...
                SVPropVals(
                    ...
                    pvs=[
                        ...
                        PropVal("povertyStatus", "BelowPovertyLevelInThePast12Months"),
                    ],
                    measured_property="",
                ),
                {
                    "povertyStatus": "State of poverty",
                    "BelowPovertyLevelInThePast12Months": "BelowPovertyLevel in the last year",
                },
                "Below Poverty Level in the last year",
            ),
        ]
    )
    def test_gen_specialized_name(
        self,
        parent: SVPropVals,
        child: SVPropVals,
        dcid2name: dict[str, str],
    ...
    def test_extract_svs_with_custom_blocklist(self):
        ...
        custom_blocklist = set(["gender"])  # pass custom blocklist to exclude 'gender'
