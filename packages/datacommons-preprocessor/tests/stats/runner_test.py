...
_TEST_DATA_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "test_data", "runner"
)
...
def _test_runner(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if config_path:
            ...
            remote_entity_types_path = os.path.join(
                input_dir, "remote_entity_types.json"
            )
        ...
        output_nl_sentences_path = os.path.join(
            temp_dir, constants.NL_DIR_NAME, constants.SENTENCES_FILE_NAME
        )
        expected_nl_sentences_path = os.path.join(
            expected_dir, constants.NL_DIR_NAME, constants.SENTENCES_FILE_NAME
        )
        output_db_path = os.path.join(temp_dir, "datacommons.sql")
        expected_db_path = os.path.join(expected_dir, "datacommons.sql")
        output_topic_cache_json_path = os.path.join(
            temp_dir, constants.NL_DIR_NAME, constants.TOPIC_CACHE_FILE_NAME
        )
        ...
        if is_write_mode():
            ...
            if os.path.exists(output_topic_cache_json_path):
                shutil.copy(
                    output_topic_cache_json_path, expected_topic_cache_json_path
                )
            return

        write_full_db_to_file(db_path=db_path, output_path=output_db_path)
        compare_files(test, output_db_path, expected_db_path, f"{test_name}: database")
        if os.path.exists(expected_nl_sentences_path):
            compare_csv_files(
                test,
                output_nl_sentences_path,
...
class TestRunner(unittest.TestCase):
    ...
    def test_config_driven_invalid(self):
        with self.assertRaises(ValueError):
            _test_runner(
                self,
                "config_driven",
                config_path=os.path.join(_CONFIG_DIR, "config_driven_invalid.json"),
            )
    ...
    def test_config_with_wildcards(self):
        _test_runner(
            ...
            config_path=os.path.join(_CONFIG_DIR, "config_with_wildcards.json"),
        )
    ...
    def test_missing_config(self):
        with self.assertRaises(FileNotFoundError):
            _test_runner(
                self, "empty", config_path=os.path.join(_CONFIG_DIR, "nonexistent.json")
            )
    ...
    def test_with_subdirs_excluded(self):
        _test_runner(
            self,
            "with_subdirs",
            config_path=os.path.join(_CONFIG_DIR, "config_exclude_subdirs.json"),
    ...
    def test_with_subdirs_included(self):
        _test_runner(
            self,
            "with_subdirs",
            config_path=os.path.join(_CONFIG_DIR, "config_include_subdirs.json"),
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for filename in os.listdir(src_dir):
                if filename != "config.json":
                    with (
                        open(os.path.join(src_dir, filename), "r") as src,
            ...
            strict_config = {
                "inputFiles": [
                    ...
                    {"pattern": "*.mcf", "provenance": "dcid:Provenance1"},
                ]
            }
    ...
    def test_read_configs_from_subdirs(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            oecd_config = {
                "inputFiles": [{"pattern": "data.csv", "importType": "OBSERVATIONS"}]
            }
            ilo_config = {
                "inputFiles": [{"pattern": "data.csv", "importType": "OBSERVATIONS"}]
            }
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            oecd_config = {
                ...
                "inputFiles": [
                    ...
                    {"pattern": "*.mcf", "provenance": "dcid:oecd"},
                ],
            }
            with open(os.path.join(oecd_dir, "config.json"), "w") as f:
                json.dump(oecd_config, f)
            ...
            ilo_config = {
                ...
                "inputFiles": [
                    ...
                    {"pattern": "*.mcf", "provenance": "dcid:ilo"},
                ],
            }
            with open(os.path.join(ilo_dir, "config.json"), "w") as f:
                json.dump(ilo_config, f)
            ...
            from datacommons_preprocessor.stats.runner import (
                create_store as real_create_store,
            )
            ...
            with (
                mock.patch(
                    "datacommons_preprocessor.stats.runner.create_store",
            ...
            self.assertTrue(
                any(
                    b.startswith("jsonld/run_timestamp/ilo/node-") for b in called_blobs
                )
            )
    ...
    def test_dcp_bridge_header_validation_failure_handshake(self):
        ...
        with mock.patch(
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                mock_create_store.assert_called_with(
                    expected_handshake_path, create_if_missing=True, treat_as_file=True
                )
