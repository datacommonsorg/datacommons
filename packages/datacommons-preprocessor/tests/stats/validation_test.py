...
class TestMetadataValidator(unittest.TestCase):
    def test_validation_success(self):
        ...
        mock_config.data = {
            "inputFiles": [{"pattern": "data.csv", "provenance": "dcid:MyProvenance"}]
        }
    ...
    def test_validation_missing_provenance(self):
        ...
        mock_config.data = {
            "inputFiles": [
                {"pattern": "data.csv", "provenance": "dcid:MissingProvenance"}
            ]
        }
        ...
        self.assertIn(
            "referenced provenances are not defined in your MCF files",
            str(context.exception),
        )
        self.assertIn("dcid:MissingProvenance", str(context.exception))
        self.assertEqual(
            context.exception.error_type, ValidationErrorType.MISSING_PROVENANCE
        )
    ...
    def test_validation_missing_source_link(self):
        ...
        self.assertIn(
            "Linked sources are missing for defined provenances", str(context.exception)
        )
        self.assertIn("has no linked Source", str(context.exception))
        self.assertEqual(
            context.exception.error_type, ValidationErrorType.MISSING_SOURCE
        )
    ...
    def test_validation_missing_provenance_key(self):
        ...
        self.assertEqual(
            context.exception.error_type, ValidationErrorType.INVALID_CONFIGURATION
        )
    ...
    def test_validation_invalid_provenance_format(self):
        ...
        mock_config.data = {
            "inputFiles": [
                {"pattern": "data.csv", "provenance": "InvalidProvenanceName"}
            ]
        }
