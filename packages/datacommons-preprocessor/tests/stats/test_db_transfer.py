...
def test_transfer_validates_sqlite_exists():
    ...
    with pytest.raises(FileNotFoundError):
        transfer_sqlite_to_cloud_sql(
            sqlite_path="/nonexistent/db.sqlite", cloud_sql_engine=mock_engine
        )
...
def test_transfer_reads_all_tables():
    ...
    """)
        ...
        result = transfer_sqlite_to_cloud_sql(
            sqlite_path=db_path, cloud_sql_engine=mock_engine
        )
...
def test_transfer_validation_failure():
    ...
    """)
        ...
        with pytest.raises(RuntimeError, match="Count mismatch"):
            transfer_sqlite_to_cloud_sql(
                sqlite_path=db_path, cloud_sql_engine=mock_engine, expected_obs=100
            )
