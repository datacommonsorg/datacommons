...
def test_get_node_model_batches_bug():
    ...
    node1.outgoing_edges = [EdgeModel(subject_id="node1", predicate="p", object_id=f"obj{i}") for i in range(5)]
    
    node2 = NodeModel(subject_id="node2", types=["TypeA"])
    # 5 edges + 1 node = 6 items
    node2.outgoing_edges = [EdgeModel(subject_id="node2", predicate="p", object_id=f"obj{i}") for i in range(5)]
    
    node3 = NodeModel(subject_id="node3", types=["TypeA"])
    # 5 edges + 1 node = 6 items
    node3.outgoing_edges = [EdgeModel(subject_id="node3", predicate="p", object_id=f"obj{i}") for i in range(5)]
