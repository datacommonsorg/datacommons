...
def test_init(mock_session, mock_config, mock_spanner_client):
    ...
    mock_spanner_client.instance.return_value.database.assert_called_once_with("test-db")

def test_get_graph_nodes(graph_service, mock_session):
    # Setup mock data
    mock_node = NodeModel(subject_id="test_node", types=["TestType"])
    mock_edge = EdgeModel(
        subject_id="test_node",
        predicate="test_predicate",
        object_id="test_target"
    )
    ...
    # Handle type filter
    mock_query.filter.return_value.params.return_value.options.return_value.limit.return_value.all.return_value = [mock_node]
    ...
    assert result.graph[0].model_dump(by_alias=True, exclude_none=True)["test_predicate"] == {"@id": "test_target"}
...
def test_insert_graph_nodes_error(graph_service, mock_spanner_client):
    ...
    mock_jsonld = JSONLDDocument(context={}, graph=[GraphNode(**{"@id": "n1", "@type": "t1"})])
