...
def _setup_toy_frog(bucket_name: str, frog_data_dir: Path) -> None:
    log_step("Setting up Toy Frog dataset...")
    csv_path = frog_data_dir / "observations.csv"
    mcf_path = frog_data_dir / "schema.mcf"
    prov_path = frog_data_dir / "provenance.mcf"
    config_path = frog_data_dir / "config.json"
    if not (
        csv_path.exists()
        and mcf_path.exists()
        and prov_path.exists()
        and config_path.exists()
    ):
        raise FileNotFoundError(f"Frog Dataset files not found under {frog_data_dir}")

    # Copy files directly to toy-frog folder
    for filename in ["observations.csv", "schema.mcf", "provenance.mcf", "config.json"]:
        run_command(
            [
                "gcloud",
                "storage",
                "cp",
                str(frog_data_dir / filename),
                f"gs://{bucket_name}/ingestion/input/toy-frog/{filename}",
            ],
            retries=3,
        )


def setup_datasets_in_gcs(bucket_name: str, test_data_dir: Path) -> None:
    """Configures and uploads Toy Frog dataset to GCS."""
    log_step(
        f"Configuring and uploading Toy Frog dataset to GCS bucket: gs://{bucket_name}/..."
    )
    _setup_toy_frog(bucket_name, test_data_dir / "frog_data")
    log_success("Toy Frog dataset successfully configured and uploaded to GCS.")
...
def main() -> None:
    ...
    try:
        ...
        # 5. Configure and upload all datasets to GCS
        test_data_dir = Path(__file__).resolve().parent / "test_data"
        setup_datasets_in_gcs(outputs["bucket_name"], test_data_dir)
