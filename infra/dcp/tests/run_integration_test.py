...
def main() -> None:
    ...
    parser.add_argument(
        "--namespace", default=None, help="Custom namespace (defaults to itest-XXXX)"
    )
    parser.add_argument(
        "--services-image",
        default=None,
    ...
    try:
        ...
        for line in lines:
            ...
            if (
                "spanner_processing_units" in line
                or "spanner_create_bigquery_reservation" in line
            ):
                continue
        ...
        if args.preprocessing_image:
            new_lines.append(
                f'ingestion_preprocessing_job_image = "{args.preprocessing_image}"\n'
            )
        if args.helper_image:
            new_lines.append(
                f'ingestion_helper_service_image = "{args.helper_image}"\n'
            )
        ...
        log_step(
            "Provisioning infrastructure via Terraform (this can take 5-7 minutes)..."
        )
        ...
        if not (csv_path.exists() and mcf_path.exists() and config_path.exists()):
            raise FileNotFoundError(
                f"Frog Dataset files not found under {frog_data_dir}"
            )
        ...
        log_step(
            f"Uploading files to GCS bucket: gs://{bucket_name}/ingestion/input/..."
        )
        run_command(
            [
                "gcloud",
                ...
                "cp",
                str(csv_path),
                f"gs://{bucket_name}/ingestion/input/observations.csv",
            ]
        )
        run_command(
            [
                ...
                "cp",
                str(mcf_path),
                f"gs://{bucket_name}/ingestion/input/schema.mcf",
            ]
        )
        run_command(
            [
                ...
                "cp",
                str(config_path),
                f"gs://{bucket_name}/ingestion/input/config.json",
            ]
        )
        ...
        run_command(
            ["uv", "run", "datacommons", "admin", "init-db", "--init-only"],
            cwd=sandbox_dir,
        )
        ...
        run_command(
            ["uv", "run", "datacommons", "admin", "ingest", "start"], cwd=sandbox_dir
        )
        ...
        while (
            workflow_state in ["ACTIVE", "RUNNING", "PENDING"]
            ...
            try:
                ...
                if executions:
                    ...
                    print(
                        f"  [Poll {elapsed}s] Workflow status: {workflow_state}",
                        flush=True,
                    )
                else:
                    print(
                ...
                print(
                    f"  [Poll {elapsed}s] Error parsing workflow status: {e}",
                    flush=True,
                )
                ...
                if desc_proc.stdout:
                    try:
                        ...
                        if error_details:
                            ...
                            print(json.dumps(error_details, indent=2), flush=True)
                            print(
                                f"\n==================================================================\n",
                                flush=True,
                            )
        ...
        if rows_count < 8:
            raise RuntimeError(
                f"Expected at least 8 observations, but found {rows_count} in Spanner."
            )
