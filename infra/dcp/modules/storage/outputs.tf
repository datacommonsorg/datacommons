...
output "pipeline_bucket_name" {
  value = var.enable_platform_service && var.deploy_pipeline ? (var.create_pipeline_bucket ? google_storage_bucket.pipeline_bucket[0].name : local.pipeline_bucket_name) : null
}
